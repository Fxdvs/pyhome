        ...
        try:
            ...
            try:
                ...
                if data:
                    ...
                    print(
                        f"from {GREEN}{server_info['host']}:{server_info['port']}@{server_info['name']}{RESET} {data}")
            except socket.timeout:
                pass
            ...
            if is_connected:
                match input("> ").strip().lower():
                    case "list":
                        ...
                        print(
                            " " * 5 + f"{GREEN}{server_info['host']}:{server_info['port']}@{server_info['name']}{RESET}\n")
                    case "clear" | "cls":
                    ...
                    case "info" | "self" | "about":
                        ...
                        print(
                            " " * 9 + f" Version: {server_info.get('version', 'Unknown')}")
                    case "send":
                        print("\n" + " " * 5 + "Connected server")
                        print(
                            f" " * 5 + f"{GREEN}{server_info['host']}:{server_info['port']}@{server_info['name']}{RESET}\n")
                        cmd = input(f"> (message) ").strip()
                        if cmd:
                            print(
                                f"> sent {GREEN}{server_info['host']}:{server_info['port']}@{server_info['name']}{RESET} {cmd}")
                    ...
                    case "color":
                        print(
                            "To enable color, type 'color enable'. To disable color, type 'color disable'.")
                    case _:
